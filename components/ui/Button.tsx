...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ variant = 'primary', size = 'md', loading = false, className, children, disabled, ...props }, ref) => {
    ...
    const variants = {
      primary: 'bg-accent text-white hover:bg-accent-600 active:scale-95',
      secondary: 'bg-primary text-white hover:bg-primary-800 active:scale-95',
      outline: 'bg-transparent border-2 border-primary text-primary hover:bg-primary hover:text-white active:scale-95',
      text: 'bg-transparent text-primary hover:text-accent active:scale-95',
    };
